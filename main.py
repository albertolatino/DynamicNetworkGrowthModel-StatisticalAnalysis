...
# Defining parameters
n0 = 10  # Initial number of vertices
m0 = 10  # Initial number of edges of a new vertex
times = [1, 10, 100, 1000]
tmax = 1000
experiments = 5
...
for i in range(1, experiments):
    ...
    if i == 1:
        test_preferential = Graph.Barabasi(n=1000, m=30, directed=False)
        deg_seq1 = []
        nodes = test_preferential.vs.indices
        for node in nodes:
            deg_seq1.append(test_preferential.vs[node].degree())
        write_file(deg_seq1, "deg_sequence_preferential.txt")
        write_file(deg_seq2, "deg_sequence_random.txt")
        write_file(deg_seq3, "deg_sequence_no_growth.txt")
